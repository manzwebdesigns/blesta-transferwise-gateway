 ...
 *
 * @package blesta
 * @subpackage blesta.components.gateways.nonmerchant.transfer_wise
 * @author Manz Web Designs, LLC
 * @copyright Copyright (c) 2020, Manz Web Designs, LLC
 * @license http://www.blesta.com/license/ The Blesta License Agreement
 * @link http://www.blesta.com/ Blesta
 * @link https://manzwebdesigns.com/ Manz Web Designs, LLC
 */
class TransferWise extends NonmerchantGateway
{
    /**
    ...
    public function __construct()
    {
        ...
        Language::loadLang('TransferWise', null, dirname(__FILE__) . DS . 'language' . DS);
    }
    ...
    public function install()
    {
        ...
        if (!extension_loaded('libxml')) {
            $errors = [
                'libxml' => [
                    'required' => Language::_('transfer_wise.!error.libxml_required', true)
                ]
            ];
    ...
    public function editSettings(array $meta)
    {
        ...
        $rules = [
            'user_email' => [
                'valid' => [
                    'rule' => ['isEmail', false],
                    'message' => Language::_('transfer_wise.!error.user_email.valid', true)
                ]
            ],
            'secret_word' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('transfer_wise.!error.secret_word.empty', true)
                ]
            ],
            'merchant_id' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('transfer_wise.!error.merchant_id.empty', true)
                ]
            ],
            'mqi' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('transfer_wise.!error.mqi.empty', true)
                ]
            ]
        ];
    ...
    {
        // Load the TransferWise API
        Loader::load(dirname(__FILE__) . DS . 'lib' . DS . 'TransferWise_api.php');
        ...
        $notification_url = Configure::get('Blesta.gw_callback_url')
            . Configure::get('Blesta.company_id') . '/TransferWise/'
        ...
        $fields = [
            ...
            'detail1_text' =>Language::_('transfer_wise.invoice.description', true),
        ...
        $this->view->set('post_url', $TransferWise->getPaymentUrl());
    ...
    public function validate(array $get, array $post)
    {
        ...
        $valid = $TransferWise->validateResponse($post);
    ...
    public function refund($reference_id, $transaction_id, $amount, $notes = null)
    {
        ...
        if (empty($this->meta['mqi'])) {
            ...
            $refund_url = $TransferWise->getRefundUrl();
            // Prepare refund request to get SID
            $this->log($this->ifSet($refund_url), serialize($fields), 'input', true);
            $TransferWise->prepareRequest($fields, 'refund');

            // Get response of the prepared request
            $response = $TransferWise->getRefundResponse();
            $error = $TransferWise->getError();
            ...
            if (!empty($response) && isset($response['sid'])) {
                ...
                $this->log($this->ifSet($refund_url), serialize($fields), 'input', true);
                $TransferWise->prepareRequest(null, 'refund', $response['sid']);
